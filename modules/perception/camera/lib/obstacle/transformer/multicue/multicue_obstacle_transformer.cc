...
void MultiCueObstacleTransformer::SetObjMapperOptions(
  ...
  *theta_ray =
  ...
  AINFO << "#2D-to-3D for one obj:";
  AINFO << "Obj pred ry:" << rotation_y;
  AINFO << "Obj pred type: " << static_cast<int>(sub_type);
  AINFO << "Bbox: " << bbox2d[0] << ", " << bbox2d[1] << ", " << bbox2d[2]
         << ", " << bbox2d[3];
}
...
void MultiCueObstacleTransformer::FillResults(
  ...
  AINFO << "Obj id: " << obj->track_id;
  AINFO << "Obj type: " << static_cast<int>(obj->sub_type);
  AINFO << "Obj ori dimension: " << obj->size[2] << ", " << obj->size[1]
         << ", " << obj->size[0];
  ...
  AINFO << "Dimension hwl: " << dimension_hwl[0] << ", " << dimension_hwl[1]
         << ", " << dimension_hwl[2];
  AINFO << "Obj ry:" << rotation_y;
  AINFO << "Obj theta: " << obj->theta;
...
bool MultiCueObstacleTransformer::Transform(
  ...
  if (frame->detected_objects.empty()) {
    AINFO << "No object input to transformer.";
    return true;
  }
  ...
  AINFO << "Camera k matrix input to transformer: \n"
  ...
  for (auto &obj : frame->detected_objects) {
    if (obj == nullptr) {
      AINFO << "Empty object input to transformer.";
      continue;
    }
